...
Client-side routing enables websites to be built that can potentially load elements much faster than traditional server-side routed sites. This is because "pages" are actually components that are rendered when a URL is chosen (by clicking on a link). The browser doesn't navigate to the URL and load a new page. Instead, JavaScript modifies the URL in the browser's address bar and optionally, the history and then an already-downloaded component is loaded. The whole screen doesn't need to be refreshed, only the section containing the component.

A possible drawback is initial load time, as the entire site might need to be loaded when first visited. All subsequent navigation happens locally from that point, at high speed.
